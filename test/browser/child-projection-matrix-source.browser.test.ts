...
@Component({ selector: "camado-matrix-child-src" })
class MatrixChildSrc extends BaseComponent {
...
@Component({ selector: "camado-matrix-slot-child-src" })
class MatrixSlotChildSrc extends BaseComponent {
	@Children({ optional: true })
	children?: ComponentChildren;
...
@Component({ selector: "camado-matrix-parent-src" })
class MatrixParentSrc extends BaseComponent {
	@Children({ optional: true })
	children?: ComponentChildren;
...
void MatrixChildSrc;
void MatrixSlotChildSrc;
void MatrixParentSrc;

test("child projection matrix from source imports", async () => {
	const parentWithText = MatrixParentSrc.create({
		children: MatrixChildSrc.create({ children: "Hoa" }),
	} as any);
	const parentWithFragment = MatrixParentSrc.create({
		children: MatrixChildSrc.create({ children: ["A", "B"] }),
	} as any);
	const parentWithComponentContent = MatrixParentSrc.create({
		content: MatrixChildSrc.create({ children: "Hoa" }),
	} as any);
	const parentWithSlot = MatrixParentSrc.create({
		footer: MatrixChildSrc.create({ children: "Footer" }),
	} as any);
	const parentWithNestedSlotChild = MatrixParentSrc.create({
		footer: MatrixSlotChildSrc.create({ children: "Slot" }),
	} as any);
	...
	const cases = Array.from(document.querySelectorAll("camado-matrix-child-src, camado-matrix-slot-child-src"));
